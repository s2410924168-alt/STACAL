...
function descNums(v){return String(v??'').trim().split(/[,;\s]+/).map(Number).filter(Number.isFinite)}
...
function calcDesc(){
 ...
 let a=[],groups=[],st;
 if(type==='raw'){
  const el=document.getElementById('data');a=descNums(el?.value||'');if(a.length<2){out.innerHTML='<div class="bi-warn">Please enter at least two numerical observations.</div>';return;}const k=Number(document.getElementById('numClasses')?.value)||Math.max(2,Math.ceil(Math.sqrt(a.length)));groups=descGroupsFromRaw(a,k);
 ...
 const rawRows=type==='raw'?[null,st.raw(1),st.raw(2),st.raw(3),st.raw(4)]:[null,st.raw(1),st.raw(2),st.raw(3),st.raw(4)];const centralRows=[0,st.mu2,st.mu3,st.mu4];
